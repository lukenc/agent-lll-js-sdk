...
async function example3_standalone() {
  ...
  const strategy = new PlanAndExecuteStrategy({
    ...
    onStepStart: (i, desc) => console.log(`▶ 步骤 ${i + 1}: ${desc}`),
    onStepComplete: (i, ok, result) => {
      const icon = ok ? '✅' : '❌'
      console.log(`${icon} 步骤 ${i + 1}: ${result?.substring(0, 100)}...`)
    },
    onPlanRevised: (steps) => console.log(`🔄 计划修订，剩余 ${steps.length} 步`),
  })

  const { content, plan } = await strategy.execute('读取 package.json 和 README.md，总结项目信息')
  console.log('\n最终结果:', content)
  console.log('\n步骤摘要:')
  plan.forEach(s => console.log(`  ${s.status === 'completed' ? '✅' : '❌'} ${s.description}`))
}
